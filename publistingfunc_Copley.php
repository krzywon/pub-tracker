...
function pubList($year, $instr, $filename, $press = false) {
	...
	if(!$open) {
		...
		$test = removeUnneededRows($open, $intno);
		if(empty($test)) {
			print "<ul>\r\n<li>None</li>\r\n</ul>\r\n";
		}
		else {	
			...
			foreach($list2 as $value) {
				if(!$value) {}
					...
					$issue = $item[3];
					$page = $item[4];
					$output .= "$authors, \"$title\", $journal $volume, $page ($year)\r\n";
					print "<tr><td>$i</td><td>$title</td><td>$authors</td><td>$journal</td><td>$volume</td><td>$issue</td><td>$page</td><td>$year</td>\r\n";
					$i++;
				}
			}
			print "</table>";
			check_exists($file, $output);
			append_data($file, "a", $output);
		}
	}
}

function removeUnneededRows($intno, $open)
{
	$list = explode("\r\n", $open);
